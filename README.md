...
![비서실 — 실장과 팀원 셋이 자리에서 일하는 중](docs/office.png)

이 그림은 이미지가 아니라 `office/public/scene.js`가 그리는 **아이소메트릭 SVG**다. 사람의 팔다리가 전부 DOM 노드라서, 위임을 받으면 실제로 실장 책상까지 걸어가고 일하는 동안 타자를 친다. AI로 렌더한 3D 사무실을 먼저 시도했다가 "안에 있는 사람을 못 움직인다"는 이유로 버린 자리다.
