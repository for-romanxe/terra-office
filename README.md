...
새 프로젝트를 물커톤실처럼 다루고 싶으면, 사이드바 **＋ 새 프로젝트 방**으로 이름만 정해 방을 연다. 물커톤실을 일반화한 구성(실장 + 전략 담당·코드 점검관·통합 담당, `prboard` 포함)이 폴더·md로 만들어지고 **서버 재시작 없이** 모두의 화면에 실시간으로 붙는다. 방 삭제는 헤더의 **방 삭제**로 하며, 세션·게시판·PR 기록까지 함께 정리된다.

개설·삭제는 **사장만** 할 수 있고, 삭제는 이렇게 만든 방(`project: true`)만 된다 — 기본 방(비서실·개발실·물커톤실)은 보호된다.
